...
pub fn avgprice(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    izip!(open, high, low, close).for_each(|(o, h, l, c)| match (o, h, l, c) {
        (Some(o), Some(h), Some(l), Some(c)) => builder.append_value((o + h + l + c) * 0.25),
...
pub fn medprice(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    izip!(high, low).for_each(|(h, l)| match (h, l) {
...
pub fn typprice(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    izip!(high, low, close).for_each(|(h, l, c)| match (h, l, c) {
...
pub fn wclprice(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    izip!(high, low, close).for_each(|(h, l, c)| match (h, l, c) {
        (Some(h), Some(l), Some(c)) => builder.append_value((h + l + 2.0 * c) / 4.0),
        _ => builder.append_null(),
    });
