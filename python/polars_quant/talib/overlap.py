...
def BBANDS(
    real: pl.Expr, timeperiod: int = 5, nbdevup: float = 2.0, nbdevdn: float = 2.0
) -> tuple[pl.Expr, pl.Expr, pl.Expr]:
    """BBANDS - Bollinger Bands (Upper, Middle, Lower)"""
    expr = register_plugin_function(
        args=[real],
        kwargs={"timeperiod": timeperiod, "nbdevup": nbdevup, "nbdevdn": nbdevdn},
        plugin_path=_LIB,
        function_name="bbands",
        is_elementwise=False,
    )
    return (
        expr.struct.field("bbands_upper"),
        expr.struct.field("bbands_middle"),
        expr.struct.field("bbands_lower"),
    )
...
# def MAMA(
#     real: pl.Expr, fastlimit: float = 0.5, slowlimit: float = 0.05
